...
def get_trade_log():
    ...
    if not game_id or not model or not date:
        return jsonify({'error': 'Game ID, Model, and Date are required'}), 400

    # 输出请求的参数，帮助调试
    print(f"game_id: {game_id}, model: {model}, date: {date}")

    # 查询数据库或文件中的交易记录
    trade_logs = TradeLog.query.filter_by(model=model, date=date).all()

    # 输出查询到的交易记录
    print(f"Found trade logs: {trade_logs}")

    if not trade_logs:
        return jsonify({'error': 'No trade logs found'}), 404

    # 默认选择第一条记录
    trade_log = trade_logs[0]  # 选择第一条记录
    ...
    trade_log_data = {
        ...
        'date': trade_log.date,
        'balance': trade_log.balance,  # 替换 trade_data 为 balance
        'portfolio': trade_log.portfolio,  # 可以根据需要返回其它字段
        'earning': trade_log.earning  # 替换 trade_data 为 earning
    }

    return jsonify([trade_log_data])  # 包装成列表形式返回
